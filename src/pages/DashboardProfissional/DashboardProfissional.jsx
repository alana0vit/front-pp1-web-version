...
function DashboardProfissional() {
  ...
  const atualizarStatus = async (pedidoId, novoStatus) => {
    try {
      ...
      if (novoStatus === "IN_WAITING" || novoStatus === "AGUARDANDO" || novoStatus === "3")
  ...
  const traduzirStatus = (status) => {
    ...
    if (s === "IN_WAITING" || s === "AGUARDANDO" || s === "3") return "Em Andamento";
  ...
  const pedidosFiltrados = pedidos.filter((p) => {
    ...
    if (statusFiltro !== "TODOS") {
      ...
      if (statusFiltro === "ANDAMENTO" && !(s === "IN_WAITING" || s === "AGUARDANDO" || s === "3")) return false;
  ...
  const contagem = (statusAlvo) => {
    return pedidos.filter((p) => {
      ...
      if (statusAlvo === "IN_WAITING") return s === "IN_WAITING" || s === "AGUARDANDO" || s === "3";
          ...
          {loading ? (
            ...
            <div className="requests-grid">
              {pedidosFiltrados.map((p) => {
                ...
                const estiloStatus = sAtual === '1' || sAtual === 'ABERTO' || sAtual === 'OPENED' ? 'opened' : sAtual === '3' || sAtual === 'AGUARDANDO' || sAtual === 'IN_WAITING' ? 'in_waiting' : sAtual === '0' || sAtual === 'FECHADO' || sAtual === 'CLOSED' ? 'closed' : 'rejected';
                  ...
                  >
                    ...
                    <div className="card-footer" onClick={(e) => e.stopPropagation()}>
                      {(sAtual === "OPENED" || sAtual === "ABERTO" || sAtual === "1") && (
                        <>
                          <button className="btn-action accept" onClick={() => atualizarStatus(p.id, "AGUARDANDO")}>Aceitar</button>
                          <button className="btn-action decline" onClick={() => atualizarStatus(p.id, "REJEITADO")}>Recusar</button>
                        </>
                      )}
                      {(sAtual === "IN_WAITING" || sAtual === "AGUARDANDO" || sAtual === "3") && (
                        <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              <div style={{ marginTop: '10px', borderTop: '1px solid #eee', paddingTop: '15px', display: 'flex', gap: '10px' }}>
                {String(pedidoDetalhado.demandStatus).toUpperCase() === 'ABERTO' || String(pedidoDetalhado.demandStatus).toUpperCase() === 'OPENED' || String(pedidoDetalhado.demandStatus).toUpperCase() === '1' ? (
                  <>
                    <button className="btn-action accept" style={{ flex: 1, padding: '12px' }} onClick={() => atualizarStatus(pedidoDetalhado.id, "AGUARDANDO")}>Aceitar Serviço</button>
                ...
                ) : (String(pedidoDetalhado.demandStatus).toUpperCase() === 'AGUARDANDO' || String(pedidoDetalhado.demandStatus).toUpperCase() === 'IN_WAITING' || String(pedidoDetalhado.demandStatus).toUpperCase() === '3') ? (
                  <div style={{ width: '100%' }}>
